...
/* ---------------------------------------------------------------
   ...
   잔별과는 완전히 다른 길로 갑니다. 편지는 은하에 별로 뜨지 않고,
   만든 사람에게만 닿아야 하니까요.

   받는 곳은 환경 변수로 정합니다 (.env.example 참고). 먼저 있는 것을 씁니다.
     VITE_LETTER_ENDPOINT  편지를 JSON으로 POST 할 주소
                           (Formspree 같은 폼 수신 서비스 주소, 또는 직접 만든 API)
     VITE_LETTER_EMAIL     위 주소가 없을 때 — 메일 앱을 열어 이 주소로 보내게 합니다

   둘 다 없으면 받을 곳이 없는 것이라, 실제 서비스(배포 빌드)에서는 편지 쓰기
   입구 자체를 숨깁니다. 보내지지 않는 편지를 "보냈어요"라고 할 수는 없으니까요.
   개발 중(npm run dev)에는 이 브라우저에만 저장해서 흐름을 확인할 수 있게 합니다.

   Supabase로 옮길 때는 send() 안쪽만 바꾸면 됩니다:
     await sb.from('letters').insert({ kind, text, email, page, sent_at })
     create table letters (
       id bigint generated always as identity primary key,
       kind text, text text not null, email text, page text,
       sent_at timestamptz not null default now()
     );  -- 쓰기(insert)만 허용하고 읽기는 막아 두세요 (RLS)
--------------------------------------------------------------- */
const LETTERS_KEY = 'janbyeol.letters.v1'
const LETTER_ENDPOINT = (import.meta.env.VITE_LETTER_ENDPOINT || '').trim()
const LETTER_EMAIL = (import.meta.env.VITE_LETTER_EMAIL || '').trim()

export const letters = {
  /** 편지가 어디로 가는지 — 'endpoint' | 'mail' | 'local' */
  channel: LETTER_ENDPOINT ? 'endpoint' : LETTER_EMAIL ? 'mail' : 'local',

  /** 편지 쓰기 입구를 보여줄 수 있는가 */
  get available() {
    return this.channel !== 'local' || Boolean(import.meta.env.DEV)
  },

  /**
   * 편지를 보냅니다.
   * @param {{ kind: string, kindLabel: string, text: string, email?: string }} letter
   * @returns {Promise<{ via: 'endpoint' | 'mail' | 'local' }>}
   */
  async send({ kind, kindLabel, text, email }) {
    const letter = {
      kind: kind || 'etc',
      kindLabel: kindLabel || '그냥 하고 싶은 말',
      text: text.trim(),
      email: (email || '').trim() || null,
      page: typeof window !== 'undefined' ? window.location.origin + window.location.pathname : '',
      sentAt: new Date().toISOString(),
    }

    if (this.channel === 'endpoint') {
      const res = await fetch(LETTER_ENDPOINT, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
        body: JSON.stringify({
          ...letter,
          _subject: `[잔별] ${letter.kindLabel}`, // 폼 수신 서비스에서 메일 제목으로 쓰입니다
          ...(letter.email ? { _replyto: letter.email } : {}),
        }),
      })
      if (!res.ok) throw new Error(`letter endpoint ${res.status}`)
      return { via: 'endpoint' }
    }

    if (this.channel === 'mail') {
      const subject = encodeURIComponent(`[잔별] ${letter.kindLabel}`)
      const body = encodeURIComponent(
        `${letter.text}\n\n—\n답장 받을 곳: ${letter.email || '(남기지 않음)'}\n보낸 곳: ${letter.page}`
      )
      window.location.href = `mailto:${LETTER_EMAIL}?subject=${subject}&body=${body}`
      return { via: 'mail' }
    }

    // 개발 중 — 이 브라우저에만 남깁니다
    try {
      const prev = JSON.parse(safeGet(LETTERS_KEY) || '[]')
      safeSet(LETTERS_KEY, JSON.stringify([...(Array.isArray(prev) ? prev : []), letter]))
    } catch {
      /* 무시 */
    }
    return { via: 'local' }
