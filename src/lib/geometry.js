...
export function project(point, cam, width, height) {
  ...
  return { sx: width / 2 + x1 * k, sy: height / 2 - y2 * k, k, z: z2 }
}

/** 성단이 만들어질 때, 끌려온 별이 잠시 머무는 고리 위 자리 */
export function ringSlot(center, index, total, seed = 0) {
  const angle = (index / Math.max(1, total)) * Math.PI * 2 + seed
  const radius = 135 + ((index * 37) % 80)
  return {
    x: center.x + Math.cos(angle) * radius,
    y: center.y + (((index * 53) % 100) - 50) * 1.2,
